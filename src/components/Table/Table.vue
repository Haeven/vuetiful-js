<template>
	<div class="table">
		<div v-if="tableData && tableData.rows && tableData.rows.length > 0" :style="`minWidth: ${minWidth}px; maxWidth: ${maxWidth}px`">
			...
			<!-- Table -->
			<div :class="`table ${tableBorder}`" :style="`minWidth: ${minWidth}px; maxWidth: ${maxWidth}px`">
				<!-- Table Header -->
				<div
					...
					<div
						...
						:key="j"
						:class="`table__cell flex-c-s ${tableBorder} ${j === 0 && headerInfirstColumn ? 'is-header' : '' }`"
				...
				<div class="table-body" :style="{ height: height }">
					...
					<div v-for="(tableRow, i) in tableData.rows" :key="i">
						<div
							...
							:class="`${ rowStripe && i % 2 === 0 ? 'is-striped' : ''} ${tableBorder}`"
	...
	methods: {
		...
		isEditable(rowIndex, columnIndex) {
			if (
				...
				|| this.headerInfirstRow && rowIndex === 0
				|| this.headerInfirstColumn && columnIndex === 0
			) return false;
		...
		onCheckAll(tableRow) {
			...
			this.$emit('selection-change', this.getCheckedRowDatas(true), this.getCheckedRowIndices(true), this.getCheckedRowNum(true));
		},
		/**
...
* @param {Number} rowIndex - Row index
		...
		getCheckedRowDatas(includeWhenHeaderInfirstRow = false) {
			const indices = this.getCheckedRowIndices(includeWhenHeaderInfirstRow);
			const checkedData = this.getData(indices);
			return checkedData || [];
		},
		/**
...
.table {
	...
	font-size: $table__cell--font-size;
	color: $table__cell--font-color;
	border: none;
	width: 100%;
...
.table__row {
	...
	&.show-border {
		border-right: $table--border-right-width solid $table--border-color;
	}
	&:hover {
...
.table__check--all:hover,
.table__check--row:hover {
	border-color: $blue;
}
.table__check--all.is-checked,
.table__check--row.is-checked {
	border-color: $blue;
...
.table__check.show-border,
.table__cell.show-border {
	border-left: $table--border-left-width solid $table--border-color;
}

.table__cell-content {
	box-sizing: border-box;
	min-width: $table__cell-content--min-width;
	min-height: $table__cell-content--min-height;
	outline: 0;
	text-align: $table__cell-content--text-align;
	position: relative;

	/* This will be needed if we add multi-layered cells
	width: 100%;
	height: 0px;
	overflow: visible;
	*/
...
.tools__tools--search {
	width: $tools__tools--search--width;
}
